...
for (const target of urls) {
  ...
  await p.goto(target, {timeout: 120000}); await p.waitForFunction(() => window.Luma && window.Luma.test); await p.waitForTimeout(1600); // let the initial framing settle
  ...
  await p.evaluate(() => { window.__scale.clickAt = performance.now() / 1000; }); await p.click('#listenBtn'); await p.waitForFunction(() => window.Luma.test.state().mode === 'listen'); await p.waitForTimeout(dur * 1000);
  ...
  const m = await p.evaluate(({LOOKAHEAD, STILL}) => {
    ...
    const notes = window.LUMA_SONG.notes.filter(n => !n.ignored), octave = 0; // a fresh context: the vocal octave setting is at its default
    const clickAt = window.__scale.clickAt; let events = 0, lastMove = -1e9, maxStep = 0, listenStep = 0, moving = 0, visViol = 0, planViol = 0, maxOver = 0; const eventTimes = [];
    for (let i = 1; i < rows.length; i++) { const [wall, t, lo, hi] = rows[i], [w0, , lo0, hi0] = rows[i - 1]; const d = Math.max(Math.abs(lo - lo0), Math.abs(hi - hi0));
      if (d > 1e-3) { moving++; if (wall - lastMove > STILL) { events++; eventTimes.push(+t.toFixed(1)); } lastMove = wall; maxStep = Math.max(maxStep, d); if (wall >= clickAt && wall <= clickAt + 1.5) listenStep = Math.max(listenStep, d); }
