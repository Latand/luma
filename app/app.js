...
const LEVELS={easy:{tolerance:80,slack:.12,ratio:.35,octaveFree:true,label:'легко'},normal:{tolerance:50,slack:.06,ratio:.5,octaveFree:false,label:'норм'},strict:{tolerance:35,slack:.05,ratio:.65,octaveFree:false,label:'точно'}};
...
const LOOP_LANE=12;
const s={ctx:null,stream:null,capture:null,micSource:null,worker:null,micGeneration:0,busy:false,cancel:0,mode:'idle',transport:null,sources:[],endTimer:null,nextTimer:null,bufs:new Map(),gains:null,pos:song.initialTime||0,range:{a:0,b:song.duration},rangeId:0,history:[],current:null,takes:[],takeCounter:0,pending:new Map(),awaitFinish:false,trace:null,live:null,verified:[],dirty:true,raf:0,lastDraw:0,lastUI:0,rangeLo:48,rangeHi:76,liveSmooth:null,lastSmoothT:0,toastTimer:0,edit:null,computeMs:0,windowMs:0,unsaved:false,scrub:null,lyricKey:'',resumeAt:null,tlDrag:null,seekTimer:0};
...
function error(text){$('errorText').textContent=text;$('errorBanner').hidden=false;say(text);}
...
function sync(){
 ...
 $('listenBtn').disabled=s.busy||s.awaitFinish;$('listenLabel').textContent=active?'Пауза':'Слухати';$('listenIcon').setAttribute('href',active?'#i-pause':'#i-play');$('stopBtn').disabled=!active&&!s.busy&&!s.awaitFinish;
 ...
 $('vocalBtn').setAttribute('aria-pressed',String(prefs.vocal));$('vocalLabel').textContent=prefs.vocal?'Голос':'Мінус';$('vocalBtn').title=prefs.vocal?'Оригінальний вокал звучить · натисни, щоб лишити тільки мінус (V)':'Тільки мінус: чуєш лише себе · натисни, щоб повернути вокал (V)';
 const pt=punchTarget();if(!singing&&!s.busy)$('singLabel').textContent=pt?'Дописати з '+fmt(s.pos):'Співати';$('singBtn').title=pt?'Перезаписати спробу '+String(pt.id).padStart(2,'0')+' від '+fmt(s.pos)+' (R)':'Записати нову спробу (R)';
...
function micError(e){return ({NotAllowedError:'Дозволь доступ до мікрофона в налаштуваннях браузера. Відкрий HTML в окремій вкладці або через localhost.',NotFoundError:'Мікрофон не знайдено.',NotReadableError:'Мікрофон зайнятий або недоступний у системі.',OverconstrainedError:'Обраний мікрофон недоступний.'})[e.name]||e.message||'Не вдалося підключити мікрофон.';}
...
function handleWorker(m){
 if(m.type==='pitch'){
  ...
  if(s.mode==='singing'&&smooth!==null&&m.confidence>.85){s.rangeLo=Math.min(s.rangeLo,Math.floor(smooth-2));s.rangeHi=Math.max(s.rangeHi,Math.ceil(smooth+2));}
...
function capacity(){const size=s.takes.reduce((a,t)=>a+t.blob.size,0);return s.takes.length<10&&size+(s.range.b-s.range.a)/prefs.speed*(s.ctx?.sampleRate||48000)*2<100*1024*1024;}
...
function playRegion(){// the loop region applies when the playhead is inside it; from outside, play on to the end of the song
 const inside=s.pos>=s.range.a-.01&&s.pos<s.range.b-.1;return inside?{a:s.range.a,b:s.range.b,loop:prefs.loop&&customRange()}:{a:0,b:song.duration,loop:false};}
...
async function startTransport(sing,quick=false){
 ...
 try{
  ...
  const punch=sing?punchTarget():null;if(sing&&!punch)s.trace=null;setRangeScale();if(s.pos>=song.duration-.1)s.pos=0;const region=playRegion();if(s.pos<region.a)s.pos=region.a;
...
function seekLive(t){
 ...
 if(s.mode==='review'){const tk=s.trace.take;t=clamp(t,tk.a,tk.endSong-.05);}else{t=clamp(t,0,song.duration-.05);const inside=t>=s.range.a-.01&&t<s.range.b-.1;end=inside?s.range.b:song.duration;loop=inside&&prefs.loop&&customRange()?{a:s.range.a,b:s.range.b}:null;}
...
function storeTake(meta,m){
 ...
 if(replaced){URL.revokeObjectURL(replaced.url);s.takes[s.takes.indexOf(replaced)]=take;}else s.takes.unshift(take);
...
function renderTakes(){const root=$('takesList');root.replaceChildren();for(const t of s.takes){
 ...
 if(t.stats.targetTime<=.1){const n=document.createElement('span');n.className='none';n.textContent='суворе оцінювання: фрагмент не підтверджено';n.title='Прослухай накладання та перевір мелодію-ціль. Сам факт знайденої ноти не доводить, що це головний вокал.';st.append(n);}
 const actions=document.createElement('div');actions.className='take-actions';for(const [label,fn,tip]of[['WAV',()=>saveWav(t),'Зберегти WAV'],['CSV',()=>saveCSV(t),'Зберегти CSV зі слідом нот і станом кожної точки'],['×',()=>removeTake(t),'Видалити спробу']]){const b=document.createElement('button');b.textContent=label;b.title=tip;b.onclick=fn;actions.append(b);}
 el.append(play,title,st,actions);root.append(el);
 }$('takesPanel').hidden=!s.takes.length;$('takesCount').textContent=s.takes.length;}
function removeTake(t){if(s.mode!=='idle'||s.awaitFinish||s.busy){toast('Спочатку зупини відтворення або запис.');return;}if(!t.saved&&!confirm('Спробу ще не завантажено. Видалити її з пам’яті?'))return;s.takes=s.takes.filter(x=>x.id!==t.id);URL.revokeObjectURL(t.url);if(s.trace?.take===t)s.trace=null;s.unsaved=s.takes.some(t=>!t.saved);renderTakes();sync();}
...
function bounds(){const narrow=W<720;const hud=narrow?(W<560?108:104):124;return{left:narrow?40:48,right:W-14,top:hud+8,bottom:H-(narrow?46:50),lane:0,laneTop:hud};}
...
function drawWords(t,v,x,y,b,opt){
 if(!prefs.lyrics||!song.lyrics.length)return;const font=(W<720?'11px ':'12px ')+getComputedStyle(document.body).getPropertyValue('--sans');g.font=font;g.textAlign='left';g.textBaseline='alphabetic';
 const placed=[];const rowH=15;
 for(const line of song.lyrics){if(line.a>v.b)break;if(line.b<v.a)continue;
  for(const w of line.words){if(w.a>v.b||w.b<v.a-.5)continue;const xx=x(w.a),tw=g.measureText(w.w).width;
   ...
   g.fillStyle=on?'#eef6ff':past?'#6b788c':'#aab5c8';g.fillText(w.w,xx,yy);if(on){g.fillStyle='#a1eed8';g.fillRect(xx,yy+2,tw,1.5);}
   g.globalAlpha=1;}}
}
...
function drawTimeline(t){tg.fillStyle='#0c1018';tg.fillRect(0,0,TW,TH);const w=song.waveform||[],x=v=>v/song.duration*TW,WH=TH-LOOP_LANE,cr=customRange();
 ...
 else{tg.fillStyle='#4a5468';tg.font='8px '+getComputedStyle(document.body).getPropertyValue('--sans');tg.textAlign='left';tg.textBaseline='middle';tg.fillText('повтор: тягни тут',6,WH+LOOP_LANE/2);}
...
function updateReadout(t){
 ...
 const sc=activeScore(),pct=scorePct(sc),pe=$('matchPct'),de=$('matchDetail');pe.classList.remove('good','low');
...
$('singBtn').onclick=()=>startTransport(true);$('listenBtn').onclick=()=>startTransport(false);$('stopBtn').onclick=()=>stopTransport();$('settingsBtn').onclick=()=>modal('settingsDialog');$('mixBtn').onclick=()=>modal('mixDialog');$('helpBtn').onclick=()=>modal('helpDialog');$('qualityBtn').onclick=()=>modal('qualityDialog');$('editBtn').onclick=openEditor;$('dismissError').onclick=()=>{$('errorBanner').hidden=true;};
$('matchDetail').onclick=()=>modal('helpDialog');$('prevMiss').onclick=()=>jumpMiss(-1);$('nextMiss').onclick=()=>jumpMiss(1);$('closeReview').onclick=closeTrace;$('takesToggle').onclick=()=>{prefs.takesOpen=!prefs.takesOpen;savePrefs();sync();};$('lyricsBtn').onclick=()=>{prefs.lyrics=!prefs.lyrics;savePrefs();s.lyricKey='~';resize();sync();};
...
$('loopBtn').onclick=()=>{prefs.loop=!prefs.loop;if(!prefs.loop)clearTimeout(s.nextTimer);if(prefs.loop&&!customRange())toast('Виділи фрагмент: тягни під хвилею або клавіші A / B під час прослуховування.');
 if(s.mode==='listen'&&s.transport)seekLive(now());sync();};$('speedSelect').onchange=()=>{prefs.speed=Number($('speedSelect').value);$('songMeta').textContent=fmt(song.duration)+(prefs.speed!==1?' · '+prefs.speed+'×':'');if(s.mode==='listen'){const t=now();stopTransport('speed');s.pos=t;startTransport(false);}sync();};
...
window.addEventListener('keydown',e=>{if(e.ctrlKey||e.metaKey||e.altKey||e.repeat&&!['ArrowLeft','ArrowRight'].includes(e.code)||e.target.closest('input,select,textarea')||document.querySelector('dialog[open]'))return;
 ...
 else if(e.code==='KeyA'){const t=now();if(setRange(t,Math.max(s.range.b,t+.5)))toast('Початок повтору: '+fmt(t));}else if(e.code==='KeyB'){const t=now();if(setRange(Math.min(s.range.a,t-.5),t))toast('Кінець повтору: '+fmt(t));}else if(e.code==='KeyL'){$('loopBtn').click();}else if(e.code==='KeyV'){$('vocalBtn').click();}});
